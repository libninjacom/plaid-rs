...
impl<'a> IncomeVerificationPaystubsGetRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<IncomeVerificationPaystubsGetResponse> {
        let mut r = self.http_client.client.post("/income/verification/paystubs/get");
...
impl<'a> ::std::future::IntoFuture for IncomeVerificationPaystubsGetRequest<'a> {
    type Output = httpclient::InMemoryResult<IncomeVerificationPaystubsGetResponse>;

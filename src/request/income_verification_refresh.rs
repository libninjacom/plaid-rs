...
impl<'a> IncomeVerificationRefreshRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<IncomeVerificationRefreshResponse> {
        let mut r = self.http_client.client.post("/income/verification/refresh");
...
impl<'a> ::std::future::IntoFuture for IncomeVerificationRefreshRequest<'a> {
    type Output = httpclient::InMemoryResult<IncomeVerificationRefreshResponse>;
